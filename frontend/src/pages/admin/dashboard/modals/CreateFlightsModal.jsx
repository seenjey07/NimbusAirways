import { adminIndexRoutesApi, adminIndexAircraftsApi, adminCreateFlightsApi } from "../../../../lib/admin/adminusersapi"
...
const CreateFlightsModal = () => {
  ...
  const handleSubmit = async () => {
    ...
    const modifiedFlightsData = {
      route_id: origin === "Manila" ? destination : destination === "Manila" ? origin : origin || destination,
    ...
    const handleAircraftChange = (selectedAircraftId) => {
        ...
        const selectedAircraft = aircrafts.find((aircraft) => aircraft.id === selectedId);
    ...
    <>    
        ...
        <h2 className="card-title justify-center text-secondary text-3xl font-bold">Create Flight</h2>
        ...
        <label className="form-control w-full max-w-xs">
            ...
            <select
                ...
                {aircrafts.length === 0 ? (
                    <option disabled value="">No aircraft available</option>
                ) : (
                    aircrafts.map((aircraft) => (
                    <option key={aircraft.id} value={aircraft.id}>
                        {aircraft.id} - {aircraft.model} - {aircraft.family} - {aircraft.current_route?.origin_location && aircraft.current_route?.destination_location
        ...
        <label className="form-control w-full max-w-xs">
            <div className="label">
                <span className="label-text text-secondary">Departure Date and Time</span>
            </div>
            <DateTimePicker
               onChange={(date) => {
            ...
            <label className="form-control w-full max-w-xs">
            <div className="label">
                <span className="label-text text-secondary">Arrival Date and Time</span>
            </div>
            <DateTimePicker
