...
const CreateRoutesModal = ({addAlert}) => {
    ...
    const [selectedDestinationRoute, setSelectedDestinationRoute] = useState(null);
    const [price, setPrice] = useState(null)
    const [status, setStatus] = useState(null)
    
    const sortedRouteOptions = routeOptions.slice().sort((a, b) => a.location.localeCompare(b.location));
      ...
      const handleOpenRoute = async () => {
        if (!selectedOriginRoute || !selectedDestinationRoute || !price || !status) {
        ...
        try {
          ...
          console.error('Error creating route:', error.response?.data?.errors || error.message);
        }
      };
      ...
      <div className="flex gap-10">
          ...
          <form className="w-full">
            <label className="form-control w-full max-w-xs">
              ...
              <select
                className="select select-bordered w-full max-w-xs text-black"
                value={selectedDestinationRoute ? selectedDestinationRoute.code : ""}
                onChange={(e) => {
                  const selectedOption = routeOptions.find(
                    (option) => option.code === e.target.value
                  );
